...
void loop() {
int rndx = random(0, xmax);
int rndy = random(0, ymax);
byte xdir = random(0, 2);
byte ydir = random(0, 2);

for (byte loops = 0; loops < 100; loops++) {
  if (rndx >= xmax) (xdir = 1);
  if (rndy >= ymax) (ydir = 1);
  if (rndx <= 0) (xdir = 0);
  if (rndy <= 0) (ydir = 0);

  DrawOneFrame (rndx, rndy);
  FastLED.show();   
  delay(50);

  if (xdir == 0) (rndx = (rndx + 1));
  if (xdir == 1) (rndx = (rndx - 1));
  if (ydir == 0) (rndy = (rndy + 1));
  if (ydir == 1) (rndy = (rndy - 1));
  }

}  
   
void DrawOneFrame (byte rndx, byte rndy) {
  byte dotx = rndx;
  byte doty = rndy;  
  for ( byte x = 0; x  < kMatrixWidth; x++) {
    for ( byte y = 0; y < kMatrixHeight; y++) {
      if ((x == dotx) and (y == doty)) {
